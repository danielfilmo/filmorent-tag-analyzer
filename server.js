...
app.post('/webhook/comentario-interno', (req, res) => {
  try {
    ...
    // y la Mac inyecta el contador en cada corrida para hacer cumplir el tope de 2.
    if (contactId && esDelAI) {
      const hoyStr = new Date().toISOString().slice(0, 10);
      global.__aiComs = global.__aiComs || {};
      const reg = global.__aiComs[contactId];
      global.__aiComs[contactId] = (reg && reg.dia === hoyStr) ? { dia: hoyStr, n: reg.n + 1 } : { dia: hoyStr, n: 1 };
    }
...
app.get('/comentarios-ai', (req, res) => {
  if (!process.env.REWARDS_HITOS_KEY || (req.query || {}).key !== process.env.REWARDS_HITOS_KEY) return res.status(403).json({ ok: false });
  const hoyStr = new Date().toISOString().slice(0, 10);
  const out = {};
  for (const id of String(req.query.ids || '').split(',').filter(Boolean)) {
    const r = (global.__aiComs || {})[id];
    out[id] = (r && r.dia === hoyStr) ? r.n : 0;
  }
  res.json({ ok: true, hoy: out });
});
